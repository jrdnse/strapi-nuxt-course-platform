...
const getProfile = async (provider, query, callback) => {
  ...
  switch (provider) {
    case "linkedin2": {
      ...
      await linkedin2
        .query()
        .get("me?projection=(firstName,lastName)")
        .auth(access_token)
        .request((err, res, body) => {
          if (err) {
            callback(err);
          } else {
            firstName = body.firstName.localized.en_US;
            lastName = body.lastName.localized.en_US;
          }
        });
      ...
      await linkedin2
        .query()
        .get(
          "clientAwareMemberHandles?q=members&projection=(elements*(handle~))"
        )
        .auth(access_token)
        .request((err, res, body) => {
          if (err) {
            ...
            setTimeout(() => {
              callback(null, {
                username: body.elements[0]["handle~"].emailAddress,
                email: body.elements[0]["handle~"].emailAddress,
                firstName: firstName,
                lastName: lastName,
              });
            }, 3000);
